...
use crate::{
    symbols::{
        index::SymbolIndex,
        scanner::{build_symbols, flatten_symbols},
        types::SymbolLocation,
    },
    syntax::SyntaxTree,
};
...
impl SymbolProvider {
    ...
    pub fn remove_file(
        ...
        self.index.map.retain(|_, v: &mut Vec<SymbolLocation>| !v.is_empty());
    }
    ...
    pub fn extract_symbols_from_snapshot(
        &self,
        snapshot: &SyntaxTree,
    ) -> Vec<DocumentSymbol> {
    ...
    pub fn quick_definition_from_snapshot(
        ...
        if matches.next().is_some() {
            debug!("[quick-def] '{word}' is ambiguous (multiple definitions in file), deferring to AST",);
            return None;
        }
        debug!(
