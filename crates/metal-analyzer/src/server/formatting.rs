...
use super::settings::FormattingSettings;
...
pub(crate) async fn format_document(
    ...
    let assume_filename = document
        .uri
        .to_file_path()
        .ok()
        .map(|p| p.display().to_string())
        .unwrap_or_else(|| "shader.metal".to_string());
    ...
    let formatted = match run_clang_format(&formatting_settings.command, &args, &document.text).await {
        Ok(output) => output,
        Err(FormattingError::CommandNotFound(_))
            if formatting_settings.command == "clang-format" =>
        {
...
fn clang_format_args(extra_args: &[String], assume_filename: String) -> Vec<String> {
...
async fn run_clang_format(
    ...
    if let Some(mut stdin) = child.stdin.take() {
        stdin
            ...
            .await
            .map_err(|error| FormattingError::LaunchFailed {
                command: executable.to_string(),
...
pub(crate) enum FormattingError {
    ...
    FormattingFailed { command: String, reason: String },
}
...
impl Display for FormattingError {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::LaunchFailed { command, reason } => {
                write!(f, "failed to launch {command}: {reason}")
            }
            Self::FormattingFailed { command, reason } => {
