...
use metal_analyzer::{
    CompletionProvider, DefinitionProvider, SemanticTokenProvider, metal::compiler::MetalCompiler, syntax::SyntaxTree,
};
...
fn completion_handles_realistic_include_and_member_contexts() {
    ...
    let include_items = provider.provide(Some(include_text), Position::new(0, include_text.len() as u32), None);
    assert!(
        include_items.iter().any(|item| item.insert_text.as_deref() == Some("<metal_stdlib>")),
        "expected include-header suggestion in include context"
    );

    let member_text = "position.";
    let member_items = provider.provide(Some(member_text), Position::new(0, member_text.len() as u32), None);
    let has_swizzle =
        member_items.iter().any(|item| item.label == "xyz" && item.kind == Some(CompletionItemKind::FIELD));
    assert!(has_swizzle, "expected member/swizzle suggestions for realistic member-access context");
}
...
fn semantic_tokens_cover_realistic_fixture_case() {
    ...
    assert!(!tokens.is_empty(), "expected semantic tokens for fixture-like corpus");
    assert!(tokens.len() > 8, "expected rich token stream for realistic fixture, got {}", tokens.len());
}
...
async fn edit_workflow_missing_symbol_error_disappears_after_fix() {
    ...
    let broken_diags = compiler.compile_with_include_paths(&broken_source, uri.as_str(), &include_paths).await;
    assert!(
        broken_diags.iter().any(|d| {
            d.severity == DiagnosticSeverity::ERROR
                && (d.message.contains("missing_scale") || d.message.to_lowercase().contains("no member"))
        }),
        "expected broken source to emit missing member error, got: {:?}",
        broken_diags
    );

    let fixed_diags = compiler.compile_with_include_paths(&fixed_source, uri.as_str(), &include_paths).await;
    assert!(
...
async fn edit_workflow_macro_warning_disappears_after_fix() {
    ...
    let warning_diags = compiler.compile_with_include_paths(&warning_source, uri.as_str(), &include_paths).await;
    assert!(
        warning_diags.iter().any(|d| d.message.to_lowercase().contains("redefine")),
    ...
    let fixed_diags = compiler.compile_with_include_paths(&fixed_source, uri.as_str(), &include_paths).await;
    assert!(
        !fixed_diags.iter().any(|d| d.message.to_lowercase().contains("redefine")),
