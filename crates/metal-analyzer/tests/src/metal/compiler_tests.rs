...
"#,
    ...
    fn fallback_include_paths_derives_clang_resource_include() {
        ...
        std::fs::create_dir_all(
            metal_binary
                .parent()
                .expect("fake metal binary should have parent directory"),
        )
        ...
        let signature = metal_binary
            .canonicalize()
            .expect("canonical fake metal binary");
        let fallback =
            fallback_include_paths_from_toolchain_signature(Some(&signature.display().to_string()));
