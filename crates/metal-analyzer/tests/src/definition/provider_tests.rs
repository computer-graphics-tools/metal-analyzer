...
"#;
    ...
    let result =
        resolve_local_template_parameter(&uri, &snapshot, source, usage, "BN").expect("template param should resolve");
    let NavigationTarget::Single(location) = result else {
        panic!("expected scalar response");
    };

    let definition_line = source.lines().nth(location.range.start.line as usize).expect("definition line");
    assert!(
...
"#;
    ...
    let result = resolve_by_name(&index, source_file, source, position, "iteration_limit")
    ...
    assert!(
        location.file_path.to_string_lossy().ends_with("/tmp/member_field_defs.h"),
...
fn by_name_field_tie_without_receiver_type_remains_ambiguous() {
    ...
    let result = resolve_by_name(&index, source_file, source, position, "iteration_limit");
    assert!(result.is_none(), "without receiver type info, tie should remain ambiguous");
}
...
"#;
    ...
    let defs = vec![
        ...
        SymbolDef {
            ...
            is_definition: true,
            type_name: None,
            qual_type: Some("const thread element_type &(const short, const short) const".into()),
        },
        SymbolDef {
            id: "parm-tile".into(),
            name: "tile".into(),
    ...
    assert!(
        location.file_path.to_string_lossy().ends_with("/tmp/member_owner.h"),
...
fn by_name_method_tie_without_receiver_type_stays_ambiguous_for_multiple_owners() {
    ...
    let result = resolve_by_name(&index, source_file, source, position, "element_at");
    assert!(result.is_none(), "method tie across multiple owners should remain ambiguous without receiver type",);
}

#[test]
fn project_index_fallback_prefers_graph_scoped_candidates() {
    let temp_dir = std::env::temp_dir().join(format!(
        "metal-analyzer-project-graph-fallback-test-{}",
    ...
    let main_file = temp_dir.join("main.metal");
    let near_file = temp_dir.join("near.h");
    let far_file = temp_dir.join("far.h");

    std::fs::write(&main_file, "#include \"near.h\"\n").expect("write main source");
    std::fs::write(&near_file, "// near header\n").expect("write near header");
    std::fs::write(&far_file, "// far header\n").expect("write far header");

    let include_paths = vec![temp_dir.display().to_string()];
    let source_file = main_file.display().to_string();
    let source_file_id = FileId::from_path(&main_file);

    let mk_index = |path: &std::path::Path, line: u32| AstIndex {
        defs: vec![SymbolDef {
            id: format!("id-{line}"),
        ...
        refs: Vec::new(),
        id_to_def: std::collections::HashMap::from([(format!("id-{line}"), 0)]),
        name_to_defs: std::collections::HashMap::from([("shared_symbol".to_string(), vec![0])]),
        target_id_to_refs: std::collections::HashMap::new(),
        file_to_defs: std::collections::HashMap::from([(path.display().to_string(), vec![0])]),
        file_to_refs: std::collections::HashMap::new(),
    };

    let project_index = ProjectIndex::new();
    project_index.update_file(near_file.clone(), mk_index(&near_file, 10));
    project_index.update_file(far_file.clone(), mk_index(&far_file, 200));

    let project_graph = ProjectGraph::new();
    project_graph.update_file(&main_file, "#include \"near.h\"\n", &include_paths);

    let result = resolve_from_project_index(
        &project_index,
        &project_graph,
        &source_file,
        Some(&source_file_id),
    ...
    .expect("graph-scoped lookup should resolve near candidate");

    let NavigationTarget::Single(location) = result else {
        panic!("expected single graph-scoped fallback location");
    };
    assert!(
        location.file_path.to_string_lossy().ends_with("near.h"),
        "expected graph-scoped near.h candidate, got {}",
        location.file_path.display()
    );
...
fn extract_namespace_qualifier_before_word_detects_scoped_access() {
    ...
    let qualifier = crate::definition::symbol_text::extract_namespace_qualifier_before_word(source, position, "repeat")
...
fn should_fast_lookup_system_symbol_detects_prefixes_and_namespaces() {
    ...
    assert!(
        should_fast_lookup_system_symbol(src_scoped, position_of(src_scoped, "greater_equal"), "greater_equal",),
        "recognized system namespaces should take fast system-header path"
    );

    let src_project = "const int custom_helper = 1;";
    assert!(
        !should_fast_lookup_system_symbol(src_project, position_of(src_project, "custom_helper"), "custom_helper",),
...
"#;
    let offset = find_scoped_enum_member_offset(src, "address", "repeat").expect("scoped enum member should be found");
...
"#,
    ...
    let position = position_of(source, "repeat");
    let include_paths = vec![temp_dir.display().to_string()];
    let result = resolve_fast_system_symbol_location(source, position, "repeat", &include_paths)
        .expect("fast system lookup should resolve namespaced enum symbol");
    let NavigationTarget::Single(location) = result else {
        panic!("expected scalar location for fast system lookup");
    };
    assert!(
        location.file_path.to_string_lossy().ends_with("metal_fake"),
...
fn system_builtin_header_candidates_include_dynamic_toolchain_headers() {
    ...
    std::fs::create_dir_all(nested_dynamic_header.parent().expect("nested fake header should have parent directory"))
        .expect("create nested include root");
    std::fs::write(&dynamic_header, "// synthetic metal header").expect("write dynamic header");
    std::fs::write(&nested_dynamic_header, "// synthetic nested metal header").expect("write nested dynamic header");
    ...
    assert!(
        candidates.contains(&nested_dynamic_header.canonicalize().expect("canonical nested dynamic header")),
...
fn find_word_boundary_offset_avoids_partial_identifier_matches() {
    ...
    let first = find_word_boundary_offset(src, "mem_threadgroup").expect("exact token should be found");
    ...
    assert!(second_search.is_none(), "partial identifier matches should be rejected",);
}

#[test]
fn provide_resolves_metal_system_sync_symbols() {
    if !has_metal_compiler() {
        return;
    }
    let _guard = AST_DUMP_COUNTER_TEST_LOCK.lock().expect("AST dump test lock should not be poisoned");
...
"#;
    ...
    let include_paths = {
        ...
        tokio::runtime::Runtime::new().expect("tokio runtime").block_on(compiler.discover_system_includes());
        compiler.get_system_include_paths().into_iter().map(|path| path.display().to_string()).collect::<Vec<_>>()
    };
    ...
    for symbol in ["threadgroup_barrier", "mem_flags", "mem_threadgroup"] {
        ...
        assert!(
            location.file_path.to_string_lossy().contains("/metal/"),
...
#[test]
fn provide_returns_none_for_static_cast_keyword() {
    let source = r#"
kernel void k(device float* sinks [[buffer(0)]], uint tid [[thread_position_in_grid]]) {
...
"#;
    ...
    let result = rx.recv_timeout(std::time::Duration::from_secs(1)).expect("static_cast lookup should not block");

    assert!(result.is_none(), "goto-definition on static_cast should return no symbol location");
}

#[test]
fn provide_returns_none_for_control_flow_keyword_without_ast_dump() {
    let _guard = AST_DUMP_COUNTER_TEST_LOCK.lock().expect("AST dump test lock should not be poisoned");
...
"#;
    ...
    assert_eq!(after - before, 0, "language keyword lookup should not trigger AST dump work");
}

#[test]
fn concurrent_goto_definition_for_same_file_runs_single_ast_dump() {
    if !has_metal_compiler() {
        return;
    }
    let _guard = AST_DUMP_COUNTER_TEST_LOCK.lock().expect("AST dump test lock should not be poisoned");
    ...
    let unique_suffix =
        std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).expect("valid clock").as_nanos();
    let source = format!(
    ...
    let responses =
        handles.into_iter().map(|handle| handle.join().expect("worker thread should not panic")).collect::<Vec<_>>();

    for response in responses {
        assert!(response.is_some(), "all rapid navigation requests should resolve definition");
    }

    let after = super::super::compiler::ast_dump_counter();
    assert_eq!(after - before, 1, "concurrent jumps on the same document should share one AST dump build");
}

#[test]
fn concurrent_index_document_and_provide_share_single_ast_dump() {
    if !has_metal_compiler() {
        return;
    }
    let _guard = AST_DUMP_COUNTER_TEST_LOCK.lock().expect("AST dump test lock should not be poisoned");
    ...
    let index_task = {
        ...
        std::thread::spawn(move || {
            barrier.wait();
            provider.index_document(&uri, source.as_str(), include_paths.as_slice());
        })
    };
    ...
    index_task.join().expect("index task thread should not panic");
    let provide_result = provide_task.join().expect("provide task thread should not panic");
    assert!(provide_result.is_some(), "navigation request should still resolve while concurrent indexing runs");

    let after = super::super::compiler::ast_dump_counter();
    assert_eq!(after - before, 1, "index_document and provide should share one AST dump build for same source hash");
}

#[test]
fn matches_position_accepts_cursor_at_token_end_boundary() {
    assert!(matches_position("/tmp/member_method_tie.metal", 71, 20, 10, "/tmp/member_method_tie.metal", 71, 30,));
}

#[test]
fn matches_position_rejects_cursor_past_token_end_boundary() {
    assert!(!matches_position("/tmp/member_method_tie.metal", 71, 20, 10, "/tmp/member_method_tie.metal", 71, 31,));
}
