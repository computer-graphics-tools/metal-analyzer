...
use super::*;
use crate::definition::{ref_site::RefSite, symbol_def::SymbolDef};
...
fn disk_cache_roundtrip_and_invalidation() {
    ...
    save_to_root(&root, &file, "source-hash-1", &include_paths, &index);

    let loaded = load_from_root(&root, &file, "source-hash-1", &include_paths);
    assert!(loaded.is_some(), "cache should load for matching key");
    assert_eq!(loaded.expect("cache payload").defs.len(), 1);

    let stale_source = load_from_root(&root, &file, "source-hash-2", &include_paths);
    assert!(stale_source.is_none(), "cache must invalidate by source hash");

    let stale_include = load_from_root(&root, &file, "source-hash-1", &["/tmp/other".to_owned()]);
    assert!(stale_include.is_none(), "cache must invalidate by include path fingerprint");
