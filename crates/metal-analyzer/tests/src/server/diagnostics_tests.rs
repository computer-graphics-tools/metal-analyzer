...
fn filter_target_diagnostics_drops_ambiguous_relative_header_path() {
    ...
    let filtered = filter_target_diagnostics(diagnostics, Some(target), true);
    assert!(filtered.is_empty(), "relative basename-only file paths must not be attributed to an arbitrary header",);
}

#[test]
fn diagnostic_paths_match_accepts_equivalent_absolute_paths() {
    assert!(diagnostic_paths_match("/tmp/project/dir/../include/utils.h", "/tmp/project/include/utils.h",));
}
...
fn filter_attaches_cross_file_note_as_related_info() {
    ...
    let related = filtered[0].related_information.as_ref().expect("note should be attached as related_information");
    assert_eq!(related.len(), 1);
    assert_eq!(related[0].message, "related note");
    assert!(related[0].location.uri.path().ends_with("defines.h"), "related info should point to the note's file");
}
...
fn filter_suppresses_macro_redefinition_warning_and_following_note() {
    ...
    let filtered = filter_target_diagnostics(diagnostics, Some(target), false);
    assert!(filtered.is_empty(), "macro redefinition warning and trailing note should be suppressed");
}
...
fn filter_drops_orphan_note_without_primary() {
    ...
    let filtered = filter_target_diagnostics(diagnostics, Some(target), false);
    assert!(filtered.is_empty(), "orphan note with no preceding primary should not appear");
}
...
fn filter_keeps_primary_when_note_has_relative_path() {
    ...
    assert!(filtered[0].related_information.is_none(), "relative note path should be silently skipped");
}
...
fn build_workspace_scan_exclude_prefixes_expands_relative_paths_per_workspace_root() {
    ...
    let exclude_paths = vec!["external/vendor-shaders".to_string(), "/opt/generated".to_string()];
...
fn is_path_excluded_matches_prefix_descendants() {
    ...
    assert!(is_path_excluded(Path::new("/tmp/ws/external/vendor-shaders/shaders/kernel.metal"), &excluded));
    assert!(!is_path_excluded(Path::new("/tmp/ws/crates/app/kernel.metal"), &excluded));
}
